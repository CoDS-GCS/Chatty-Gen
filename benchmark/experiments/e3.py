...
logger_obj = Logger()
logger_obj.configure_logger(file_path="logs/e3.log")
...
N_Q_PROMPT = PromptTemplate(
    ...
    template="""Generate a list of n questions based on a subgraph schema from a knowledge graph. Each question should relate to a shared entity (e) within the subgraph and should fall into one of the following categories: list, count, boolean, wh (open-ended), or date-related questions. Each question should be answerable solely from the information in the provided subgraph without explicitly mentioning it. The questions can be equivalent to one or two triplets from the subgraph.
{format_instructions}
...
def create_schema(subgraph, seedtype="publication"):
    if seedtype == "creator":
        author = subgraph[0]['subject']
        schema = {
            "seed": f"{author}",
            "type": "Creator",
            "predicates": [("primary affiliation", "xml:string"), ("Publication", "authored by"), ("author of", "Publication")]
        }
    elif seedtype == "publication":
        publication = subgraph[0]['subject']
        schema = {
            "seed": f"{publication}",
            "type": "Publication",
            "predicates": [("author by", "Creator"), ("Publication", "authored by"), ("Creator", "author of")]
        }
    return schema
        ...
        if len(subgraph) > 0:
            ...
            try:
                with get_openai_callback() as cb:
                    ...
                    logger.info(f"INDEX : {idx} -- schema input : {schema} --")
