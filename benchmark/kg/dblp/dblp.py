...
DBLP_ENDPOINTS = ["https://sparql.dblp.org/sparql"]
MAX_WAIT_TIME = 1.0
...
class DBLP:
    def __init__(self, _method="round-robin", _verbose=False, _db_name=0):
        ...
        print(os.path.join(CURR_DIR, "dblp_rdf_schema.nt"))
        self.schema = KgSchema(
            os.path.join(CURR_DIR, "dblp_rdf_schema.nt"), rdf_format="nt"
        )
    ...
    def extract_subgraphs(self, seed_nodes):
        ...
        for node_type, node_info in parsed_schema_map.items():
            ...
            for seed_node in seeds:
                ...
                self.subgraphs_map[node_type].append(subgraph)
        return self.subgraphs_map
