...
def get_answer_LLM_based(question, triples_list, subgraph, approach, trace_input=None, trace=None):
    # triples_list = list()
    # for triple in triples:
...
def get_LLM_based_postprocessed(question, triples_list, subgraph, approach, trace_input=None, trace=None):
    llm_query = get_answer_LLM_based(question, triples_list, subgraph, approach, trace_input=trace_input, trace=trace)
    if is_boolean(question) and not llm_query.lower().startswith("ask"):
        ...
        # why is it only used for subgraph???
        is_count_query = detect_is_count_query(question, triples_list)
