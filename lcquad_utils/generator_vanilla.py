...
list_of_entities = open('resources/entities_short.txt').read().split('\n')                # Contains list of entites for which the question would be asked
...
for line in open('resources/relations_with_probability.txt'):
    prop,p = line.split()
    probability_property[prop] = float(p)
entity_went_bad = []

templates = json.load(open('sparql_templates.json'))  # Contains all the templates existing in templates.py
...
def fill_templates_1(_graph, _uri):
    '''
        This function is programmed to traverse through the Subgraph and create mappings for templates
...
def generate_answer(_uri, dbp):
    try:
        ...
        fill_templates_1(graph, _uri=uri)
...
with open('resources/properties_count.pickle', 'wb') as file:
    pickle.dump(properties_count, file)
print("DONE")

def open_file_with_parent_validation(file_path):
    try:
        # Ensure the parent directories exist
        parent_directory = os.path.dirname(file_path)
        os.makedirs(parent_directory, exist_ok=True)

        # Determine the file mode (w+ if the file doesn't exist, a+ if it does)
...
for key in sparqls:
    writepath = 'sparqls/template%d.txt' % key
    fo = open_file_with_parent_validation(writepath)
    if fo is not None:
