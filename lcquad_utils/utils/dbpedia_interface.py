...
import utils.labels_multiple_form as lmultiform
...
GET_CLASS_PATH = (
    """SELECT DISTINCT ?type WHERE { %(target_class)s rdfs:subClassOf* ?type }"""
)
...
class DBPedia:
    ...
    def get_properties_on_resource(self, _resource_uri):
        ...
        response = self.shoot_custom_query(
            GET_PROPERTIES_ON_RESOURCE % {"target_resource": _resource_uri}
        )
    ...
    def get_properties_of_resource(self, _resource_uri, _with_connected_resource=False):
        ...
        if _with_connected_resource:
            temp_query = GET_PROPERTIES_OF_RESOURCE_WITH_OBJECTS % {
        ...
        try:
            if _with_connected_resource:
                property_list = [
                    [
                        x["property"]["value"].encode("ascii", "ignore"),
                        x["resource"]["value"].encode("ascii", "ignore"),
                    ]
    ...
    def get_entities_of_class(self, _class_uri):
        ...
        response = self.shoot_custom_query(
            GET_ENTITIES_OF_CLASS % {"target_class": _class_uri}
        )

        try:
            entity_list = [
                x["entity"]["value"].encode("ascii", "ignore")
                for x in response["results"]["bindings"]
            ]
        except:
            # TODO: Find and handle exceptions appropriately
    ...
    def get_type_of_resource(self, _resource_uri, _filter_dbpedia=False):
        ...
        _resource_uri = "<" + _resource_uri + ">"
        response = self.shoot_custom_query(
            GET_TYPE_OF_RESOURCE % {"target_resource": _resource_uri}
        )
        try:
            type_list = [
                x["type"]["value"].encode("ascii", "ignore")
                for x in response["results"]["bindings"]
            ]
        except:
            traceback.print_exc()
        ...
        if _filter_dbpedia:
            filtered_type_list = [
                ...
                for x in type_list
                if x[:28]
                in ["http://dbpedia.org/ontology/", "http://dbpedia.org/property/"]
            ]
    ...
    def get_answer(self, _sparql_query):
        ...
        variables = [x for x in response["head"]["vars"]]

        # NOTE: Assuming that there's only one variable
        values = {}
        for index in range(0, len(variables)):
            # value = [
            #     x[variables[index]]["value"].encode("ascii", "ignore")
            #     for x in response["results"]["bindings"]
            # ]
            value = [
                x[variables[index]]["value"]
                for x in response["results"]["bindings"]
            ]
    ...
    def get_label(self, _resource_uri):
        ...
        try:
            ...
            try:
                response = self.shoot_custom_query(
                    GET_LABEL_OF_RESOURCE % {"target_resource": _resource_uri}
                )

                results = [
                    x["label"]["value"].encode("ascii", "ignore")
                    for x in response["results"]["bindings"]
                ]
                if len(results) > 0:
    ...
    def get_most_specific_class(self, _resource_uri):
        ...
        """
        # _resource_uri = _resource_uri.decode()
        print(_resource_uri,type(_resource_uri))
        if not nlutils.has_url(_resource_uri):
            warnings.warn(
                "The passed resource %s is not a proper URI but probably a shorthand. This is strongly discouraged."
                % _resource_uri
    ...
    def is_common_parent(self, _resource_uri_1, _resource_uri_2):
        ...
        try:
            response_uri_1 = self.shoot_custom_query(
                GET_SUPERCLASS % {"target_class": specific_class_uri_1}
            )
            response_uri_2 = self.shoot_custom_query(
                GET_SUPERCLASS % {"target_class": specific_class_uri_2}
            )
        except:
            traceback.print_exc()
        ...
        try:
            results_1 = [
                x["type"]["value"].encode("ascii", "ignore")
                for x in response_uri_1["results"]["bindings"]
            ]
            results_2 = [
                x["type"]["value"].encode("ascii", "ignore")
                for x in response_uri_2["results"]["bindings"]
            ]
        except:
            traceback.print_exc()
        filtered_type_list_1 = [
            x
            for x in results_1
            if x[:28]
            in ["http://dbpedia.org/ontology/", "http://dbpedia.org/property/"]
        ]
        filtered_type_list_2 = [
            x
            for x in results_2
            if x[:28]
            in ["http://dbpedia.org/ontology/", "http://dbpedia.org/property/"]
